...
lpq-class [-g] [-c] [-h] [-i] [-l] [-m] [-p [tfhm]] [-r]


DESCRIPTION

Classifies knots up to n crossings in the infinite family of lens spaces L(p,q).
A knot is a smooth embedding of a circle S^1 to a 3-manifold.
The lens space L(p,q) is the quotient of the 3-sphere by the Z/p action generated by
(z_0, z_1) = (e^(2 pi i/p * z0, e^(2 pi i q/p * z1), with p and q being coprime integers.
The classification algorithm uses the following knot invariants:
- The Kauffman bracket skein module of the knots.
- The HOMFLYPT skein module of knots in L(p,1) in the standard basis.
- The HOMFLYPT skein module of knots in L(p,2) in the standard basis (conjectured existence).
